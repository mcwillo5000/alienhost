...
namespace Pterodactyl\BlueprintFramework\Extensions\databaseimportexport\Dependencies\Ifsnop\Mysqldump;
...
class Mysqldump
{
    ...
    const DB_USERNAME_REGEX = '/`u[0-9]+_[A-Za-z0-9]{10}`/';
    ...
    */
    public function restore($path)
    {
        ...
        while ( !feof($handle) ) {
            ...
            if ($delimiter == substr(trim($line), -strlen($delimiter), strlen($delimiter))) {
                ...
                $buffer = trim($buffer);
                $sql = substr($buffer, 0, strlen($buffer) - strlen($delimiter));
                $sql = str_replace($delimiter, ';', $sql);

                // find any mentions of DB_USERNAME_REGEX and replace with the actual username
                $sql = preg_replace(self::DB_USERNAME_REGEX, "`{$this->user}`", $sql);
    ...
    public function wipe()
    {
        ...
        $this->dbHandler->exec("SET FOREIGN_KEY_CHECKS=0");

        foreach ($this->procedures as $procedure) {
            $this->dbHandler->exec("DROP PROCEDURE IF EXISTS `{$procedure}`");
        }

        foreach ($this->functions as $function) {
            $this->dbHandler->exec("DROP FUNCTION IF EXISTS `{$function}`");
        }

        foreach ($this->events as $event) {
            $this->dbHandler->exec("DROP EVENT IF EXISTS `{$event}`");
        }

        foreach ($this->triggers as $trigger) {
            $this->dbHandler->exec("DROP TRIGGER IF EXISTS `{$trigger}`");
        }

        foreach ($this->views as $view) {
            $this->dbHandler->exec("DROP VIEW IF EXISTS `{$view}`");
        }

        foreach ($this->tables as $table) {
            $this->dbHandler->exec("DROP TABLE IF EXISTS `{$table}`");
        }

        $this->dbHandler->exec("SET FOREIGN_KEY_CHECKS=1");
    }
    ...
    public function dbSize(): int
    {
        ...
        $query = "SELECT table_schema 'database', sum(data_length + index_length) 'size' FROM information_schema.tables WHERE table_schema = '{$this->dbName}' GROUP BY table_schema";
        $stmt = $this->dbHandler->query($query);
