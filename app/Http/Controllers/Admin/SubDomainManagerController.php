...
class SubDomainManagerController extends Controller
{
    ...
    /**
     * SubDomainManagerController constructor.
    ...
    public function settings(Request $request)
    {
        ...
        if (!$this->validateCloudflareApiKey($email, $api_key)) {
            $this->alert->danger('Invalid Cloudflare API Key. Please make sure you are using the Global API Key.')->flash();
            return redirect()->route('admin.subdomain');
        }
    ...
    /**
     * Validate the Cloudflare API Key by making an API request.
     */
    private function validateCloudflareApiKey(string $email, string $api_key): bool
    {
        $client = new \GuzzleHttp\Client();
        
        try {
            $response = $client->request('GET', 'https://api.cloudflare.com/client/v4/user', [
                'headers' => [
                    'X-Auth-Email' => $email,
                    'X-Auth-Key' => $api_key,
            ...
            \Log::error('Cloudflare API Key validation failed', ['error' => $e->getMessage()]);
            return false;
        }
    }
