                        ...
                        <div css={tw`flex flex-col gap-2`}>
                            {items.length > 0 ? items.map((prefab: Prefab) => (
                                <div
                                    ...
                                    onClick={() => handleInstall(prefab.id, prefab.name)}
                                >
                                    {prefab.icon_url ? (
                                        <img src={prefab.icon_url} alt={prefab.name} css={tw`rounded w-10 h-10 object-contain flex-shrink-0`} />
                                    ) : (
                                    ...
                                    <div css={tw`flex flex-col flex-1 min-w-0`}>
                                        <div css={tw`flex items-center gap-2`}>
                                            ...
                                            {installedPrefabIds.has(prefab.id) && (
                                                <span css={tw`inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-xs`} style={{ backgroundColor: 'rgba(34,197,94,0.15)', color: '#4ade80', border: '1px solid rgba(34,197,94,0.3)' }}>
                                        ...
                                        <div css={tw`flex items-center gap-3 mt-0.5 flex-wrap`}>
                                            {prefab.author && prefab.author.trim() !== '' && (
                                                <span css={tw`text-xs`} style={{ color: 'var(--theme-text-muted)' }}>By {prefab.author}</span>
                                            )}
                                            ...
                                            <p css={tw`text-xs line-clamp-1 flex-1 min-w-0`} style={{ color: 'var(--theme-text-muted)' }}>{prefab.short_description}</p>
                                        </div>
                                    </div>
                                    <button
                                        title='Install'
                                        ...
                                        onClick={(e) => { e.stopPropagation(); handleInstall(prefab.id, prefab.name); }}
                                    >
                                        <FontAwesomeIcon icon={faDownload} />
                                    </button>
                                </div>
