...
const InstallModalComponent = ({ prefabId, prefabName, onInstalled, open, onClose }: Props) => {
    const uuid = ServerContext.useStoreState((state) => state.server.data!.uuid);
    const { clearFlashes, clearAndAddHttpError } = useFlashKey('prefabs:install-modal');
    ...
    const safePrefabId = prefabId || '';
    const safePrefabName = prefabName || 'Prefab';
    useEffect(() => {
        if (!open || !prefabId) return;
        clearFlashes();
        setError(null);
        setLoading(true);
        ...
        const safePrefabId = prefabId || '';
        try {
            http.get(`/api/client/servers/${uuid}/hytale-prefabs/${safePrefabId}/versions`)
                .then(({ data }) => {
                ...
                .catch(error => {
                    console.error('Error loading prefab versions:', error);
                    setError(httpErrorToHuman(error));
                })
                .finally(() => setLoading(false));
        } catch (error) {
            console.error('Error in InstallModal effect:', error);
            setError('An unexpected error occurred while loading prefab versions.');
            setLoading(false);
        }
    }, [prefabId, open]);
    const handleClose = () => {
    ...
    const submit = ({ version }: Values, { setSubmitting }: FormikHelpers<Values>) => {
        ...
        console.log('Installing prefab:', { prefabId: safePrefabId, version: safeVersion });
        setSubmitting(true);
        try {
            installPrefab(uuid, safePrefabId, safeVersion, project?.name || project?.title || prefabName, project?.icon_url || '', project?.author || '')
                .then(() => {
                    console.log('Prefab installed successfully');
                    setSubmitting(false);
                    handleClose();
                    onInstalled();
                })
                .catch(error => {
                    console.error('Error installing prefab:', error);
                    setError(httpErrorToHuman(error));
                    setSubmitting(false);
                });
        } catch (error) {
            console.error('Error in submit function:', error);
            setError('An unexpected error occurred while installing the prefab.');
            setSubmitting(false);
        }
    };
    return (
        <Dialog
            open={open && !isClosing}
            onClose={handleClose}
            title={`Install ${safePrefabName}`}
            hideCloseIcon={false}
        >
            <ModalContent isClosing={isClosing}>
                <FlashMessageRender byKey={'prefabs:install-modal'} css={tw`mb-4`} />
                {error && (
                ...
                {loading ? (
                    <LoadingContainer>
                        <Spinner size={'large'} />
                        <LoadingText>Loading prefab versions...</LoadingText>
                    </LoadingContainer>
                ) : versions.length === 0 ? (
                    <Alert type="warning" className="my-4">
                        <div css={tw`flex flex-col items-center w-full py-4`}>
                            ...
                            <p>No versions available for this prefab.</p>
                        </div>
                    </Alert>
                ) : (
                    <>
                        {project && (
                            <PrefabHeader>
                                {project.icon_url ? (
                                    <PrefabIcon src={project.icon_url} alt={project.name || project.title || prefabName} />
                                ) : (
                                    <PlaceholderIcon>
                                ...
                                <PrefabInfo>
                                    <PrefabTitle>{project.name || project.title || prefabName}</PrefabTitle>
                                    <PrefabDescription>
                                ...
                                <Form>
                                    <FormContainer>
                                        <div>
                                            ...
                                            <HelperText>
                                                Select the version of the prefab you want to install.
                                            </HelperText>
                                        </div>
                                        <Dialog.Footer>
                                            <Button.Text
