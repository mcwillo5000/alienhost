...
const generateAnnouncementId = (announcement: NodeAnnouncement, index: number): string => {
    const content = `${announcement.node_id}-${index}-${announcement.title}-${announcement.description}`;
...
export default () => {
    ...
    useEffect(() => {
        const announcements = (window as any).SiteConfiguration?.nodeAnnouncements || [];
        setNodeAnnouncements(announcements);
    }, []);
    ...
    useEffect(() => {
        ...
        nodeAnnouncements.forEach((announcement, index) => {
            const announcementId = generateAnnouncementId(announcement, index);
            if (getCookie(announcementId) === 'dismissed') {
    ...
    const serverNodeName = server.node;


    const matchingAnnouncements = nodeAnnouncements
        .map((announcement, index) => ({ ...announcement, _index: index }))
        .filter((announcement) => {
            const announcementId = generateAnnouncementId(announcement, announcement._index);
            return announcement.node_name === serverNodeName && !dismissedAnnouncements.includes(announcementId);
        });
