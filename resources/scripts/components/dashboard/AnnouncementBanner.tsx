...
const AnnouncementFrame: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const framePath = `
        M ${so},${so + cornerCut}
        L ${so + cornerCut},${so}
        L ${width - so - cornerCut},${so}
        L ${width - so},${so + cornerCut}
...
const SingleBanner = ({ 
    ...
    return (
        <BannerWrapper className="announcement-banner">
            <AnnouncementFrame>
                <BannerInner>
                    {/* Icon */}
                    {icon && (
...
export default ({ announcement }: AnnouncementBannerProps) => {
    const [dismissedGlobal, setDismissedGlobal] = useState(false);

    const hasGlobalAnnouncement = !!(announcement.title || announcement.description);

    if (!hasGlobalAnnouncement || dismissedGlobal) {
        return null;
    }
    ...
    return (
        <div css={tw`mb-2`}>
            ...
            <SingleBanner
                ...
                onClose={() => setDismissedGlobal(true)}
            />
        </div>
    );
};